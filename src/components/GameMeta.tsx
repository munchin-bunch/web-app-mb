...
export const GameMeta = ({ }: GameMetaProps) => {
    ...
    <div className="flex flex-col gap-4 mb-8">
      ...
      <div>
        <Box className="flex gap-3 items-center">
          <div>MAIN PRIZE:</div>
          <div className="font-bold text-xl text-blue-primary">2500 USDT & PUDGY PENGUIN NFT</div>
        </Box>
      </div>
      <div className="grid md:grid-cols-1 gap-4 lg:grid-cols-3">
        <Box>
          <div>CURRENT POOL</div>
          <div className="text-2xl font-bold text-blue-primary">{data?.prizePool}</div>
        </Box>
        <Box>
          <div>ENTRY TICKET</div>
          <div className="text-2xl font-bold text-blue-primary">{data?.betAmount}</div>
        </Box>
        <Box>
